...
export function MyBookingsPage() {
  ...
  const harborId = (user as { harbor_id?: string | null } | null)?.harbor_id ?? getHarborIdFromSlug(marinaSlug);
  ...
  const berthsMap = new Map(berths.map(b => [b.berth_id, b.label || b.berth_id]));
    ...
    <div className="min-h-screen bg-slate-50 pt-32 pb-12 px-4 md:px-10">
      <div className="mx-auto max-w-5xl">
        <header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="text-slate-500 mt-2">Manage your upcoming and past berth reservations</p>
          </div>
        </header>
        ...
        {!isLoading && bookings.length === 0 && (
          <div className="rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm">
            ...
            <h3 className="text-lg font-bold text-brand-navy">No bookings found</h3>
            <p className="text-slate-500 mt-1">Here is where all your bookings show up</p>
          </div>
        )}
        ...
        {!isLoading && bookings.length > 0 && (
          <div className="grid gap-4">
            {sortedBookings.map((booking) => {
              const berthLabel = berthsMap.get(booking.berth_id) || `Berth ${booking.berth_id.split("-").pop()}`;
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      isConfirmed && "bg-gradient-to-br from-emerald-400 to-emerald-600 shadow-emerald-200",
                      isCancelled && "bg-slate-300 shadow-none",
                      isCompleted && "bg-gradient-to-br from-brand-blue to-brand-cyan shadow-brand-blue/20"
                    )}>
                      {berthLabel.slice(0, 2)}
                    </div>
                    
                    <div>
                      <h4 className="font-black text-brand-navy">{berthLabel}</h4>
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        <span>{new Date(booking.from_date).toLocaleDateString()}</span>
                        <span>→</span>
                        <span>{new Date(booking.to_date).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </div>
      ...
      <Dialog open={!!bookingToCancel} onOpenChange={(open) => !open && setBookingToCancel(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-brand-navy">Cancel Booking?</DialogTitle>
            <DialogDescription className="pt-2 text-slate-500">
              Are you sure you want to cancel this booking? This action cannot be undone and will free up the berth for others.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-6 flex gap-2 sm:justify-end">
            <DialogClose asChild>
              <Button variant="ghost" className="font-bold">Keep Booking</Button>
            </DialogClose>
            <Button 
              variant="destructive" 
              className="font-bold shadow-lg shadow-red-200"
              onClick={() => bookingToCancel && handleCancelOrClear(bookingToCancel)}
              disabled={isCancelling}
            >
